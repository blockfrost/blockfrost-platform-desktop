...
Section "" _sec14
  ...
  StrCpy $_16 "1"
  ClearErrors
  Delete "$APPDATA\@projectCodeName@\@lockfileName@"
  IfErrors 0 _lbl33
  StrCpy $_16 ""
  Goto _lbl36
  _lbl33:
  ...
  _lbl52:
  File /r "contents\*"
  CreateShortcut "$DESKTOP\@projectName@.lnk" "$INSTDIR\@projectCodeName@.exe" "" "$INSTDIR\@projectCodeName@.exe" "0" "SW_SHOWMINIMIZED" "" ""
...
Section "Start Menu Shortcuts" _sec62
  ...
  CreateShortcut "$SMPROGRAMS\@projectName@\@projectName@.lnk" "$INSTDIR\@projectCodeName@.exe" "" "$INSTDIR\@projectCodeName@.exe" "0" "SW_SHOWMINIMIZED" "" ""
SectionEnd
Function .onInit
  ${IfNot} ${AtLeastWin8.1}
